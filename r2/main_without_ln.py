...
#   data to control looping
    ...
    mybatchSize = 32
    my_learning_rate = 0.001 #2e-4 #0.000000001 
            

    # to collect all data in one excel sheet
    from datetime import datetime               
    fDateTime = datetime.now().strftime("%Y-%m-%d _ %H_%M_%S_")
    alloutputFile = 'out\\' +'out_All_LR_' +str(my_learning_rate)+'_' + fDateTime +' .xlsx'
    # add out path
...
# starting data will be changed at looping
            ...
            #myepochs = 50
            modelName = current_input_excel_file +'.LR.'+str(my_learning_rate)+ '.layers.'+ str(layersNum) + '.epoch.' + str(myepochs)  +'.h5' #'testmodel.epoch.'+ str(myepochs) +'.h5'
            model = None    
            myOut = 'out\\'+'LR_'+str(my_learning_rate) + '\\num_layers_' +str(layersNum)+ '\\num_epochs_' + str(myepochs) + '_'
